...
**① 两个 tab 必须拿到不同的设备 ID。**
设备 ID 存在 `sessionStorage`（按 tab 隔离）而**不是** `localStorage`——
后者是同一浏览器所有 tab 共享的，两边会拿到同一个 ID，于是各自把对方的帧当成
"自己的回声"丢掉，永远配不上对。这个坑是实测复现出来的（同一个 profile 连开两次，
两次都显示"设备97"），现在已经修掉，并且加了兜底：万一 ID 还是撞了，
会检测到冲突、自动改名并重新配对。

| 双开方式 | 设备 ID 是否隔离 | 能用吗 |
|---|---|---|
| 同一浏览器开两个 tab | ✅ sessionStorage 按 tab 隔离 | ✅ |
| 两个不同浏览器（如 Chrome + Edge） | ✅ 各自独立的 localStorage | ✅ |
| 两个无痕窗口 | ✅ 独立会话 | ✅ |
